#!/bin/bash
#
# This script will generate shear modulus input scripts for the PLA systems

# Sim data files here
# sim_data_files=$(find ./ -name "PLASiO2_03172022_IFFRPlyEq_504_S0.dat" | sort)
...
### Simulation history ###
##########################
# PLA trimer was read in, minimized using cg with settings 1.0e-4 1.0e-6 1000 10000, initialize velocity while zeroing linear and angular momentum, and then run for 1 ns with NVT at 300 K
# densify 504 PLA molecules to a cube with a density of 1.32g/cc. First do NVT at 300 K for 100 ps so that the molecules come together. Second do NPT iso at 300 K and 1.0 atm for 200 ps so that the simulation box reduces in size such that it contains the cluster of molecules. Third Densify at a rate of 20 A/ns until the target density is achieved with NVT at 300 K. Finally run a density profile.
# Fix indent was used to create a spherical cavity in the system over 10 ps so that the SiO2 nanoparticle could be inserted
# insert the SiO2 NP into the system. Zero the linear velocity so that the NP does not drift. Then nve/limit 0.1 for 50 ps to move overlapping atoms off of each other. Then NVT at 300 K for 100 ps. Then NPT iso at 300 K and 1.0 atm for 500 ps.
# polymerize the PLA-SiO2 system until it is fully polymerized or until 1 ns of sim time has been completed. NPT aniso at 300 K and 1.0 atm.  Delete H2O molecules as they are produced. Turn on long-range interactions
# equilibrate the system for 8 ns at 300 K and 1.0 atm with NPT aniso.
#################################
